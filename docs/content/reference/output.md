...
describes what users get. Pick a format with `-o`, or let rubygems choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
rubygems <command> -o table   # aligned columns for reading
rubygems <command> -o jsonl   # one JSON object per line, for piping
rubygems <command> -o json    # a single JSON array
rubygems <command> -o csv     # spreadsheet friendly
rubygems <command> -o tsv     # tab-separated
rubygems <command> -o url     # just the URL column
rubygems <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
rubygems <command> --fields id,title,url
```
...
```bash
rubygems <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
rubygems <command>            # a table, because this is a terminal
rubygems <command> | wc -l    # JSONL, because this is a pipe
```
