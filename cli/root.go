// Package cli assembles the rubygems command tree from the rubygems
...
// NewApp assembles the kit application from the rubygems domain. The
...
//
// To add a command, declare it in rubygems/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := rubygems.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(rubygems.Domain{}).Register(app)
